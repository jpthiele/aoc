...
function process_beam(b::beam,mirrors::Matrix{Char})
n = size(mirrors)[1]
beams = Vector{beam}([b])
heated = fill(zero(Int8), n, n)
dirs = Matrix{Vector{Bool}}(undef,n,n)
...
for i ∈ 1:numlines
    global sol = maximum([sol,process_beam(beam(CartesianIndex(i,0),CartesianIndex(0,1)),mirrs)])
    global sol = maximum([sol,process_beam(beam(CartesianIndex(i,numlines+1),CartesianIndex(0,-1)),mirrs)])
    global sol = maximum([sol,process_beam(beam(CartesianIndex(0,i),CartesianIndex(1,0)),mirrs)])
    global sol = maximum([sol,process_beam(beam(CartesianIndex(numlines+1,i),CartesianIndex(-1,0)),mirrs)])
end
