...
# Get the maximum of cubes drawn for a specific color 
function maxcubes(line::String,pattern::Regex)::Int64
    max::Int64 = 0
...
# and get the maximum number of cubes for all three colors.
sol::Int32 = 0
open(filename) do file
    for l in eachline(file)
        gameran = findfirst(r"Game \d+", l)
        game = parse(Int64,l[6:gameran[end]])
        red = maxcubes(l,r"\d+ red")
        green = maxcubes(l,r"\d+ green")
        blue = maxcubes(l,r"\d+ blue")
        global sol += red*green*blue
    end
end
